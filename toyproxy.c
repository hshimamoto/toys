...
static int dial(const char *hostport)
{
	...
	int one = 1;
	if (ioctl(r, FIONBIO, &one) < 0) {
		close(r);
		r = -1;
		goto out;
	}

	connect(r, res->ai_addr, res->ai_addrlen);
	// returns EINPROGRESS
...
static void request(struct session *s)
{
	...
	s->n += nr;
	char *body = strstr(s->buf, "\r\n\r\n");
	if (body == NULL) {
		if (s->n == 256) {
			logf("<%02d> header too long\n", s->id);
			session_markclose(s);
			return;
		}
		return;
	...
	// request check
	if (strncmp(s->buf, "CONNECT ", 8)) {
		logf("<%02d> no CONNECT method\n", s->id);
		session_markclose(s);
		return;
	}
	// get host:port HTTP/1.x
	const char *hostport = NULL;
	for (int i = 12; i < (s->n - 1); i++) {
		if (s->buf[i] == ' ' && s->buf[i + 1] == 'H') {
			hostport = &s->buf[8];
			s->buf[i] = '\0';
			break;
		}
	}
	// okay now we can use hostport
	logf("<%02d> dial to %s\n", s->id, hostport);
	int sock = dial(hostport);
	if (sock == -1) {
		logf("<%02d> dial failed\n", s->id);
		session_markclose(s);
		return;
	}
	s->connecting = sock;
...
static void connected(struct session *s)
{
	...
	logf("<%02d> connected\n", s->id);
	// send Response
	if (write(s->dl, "HTTP/1.0 200 CONNECT OK\r\n\r\n", 27) < 0) {
		session_markclose(s);
		return;
	}
}
